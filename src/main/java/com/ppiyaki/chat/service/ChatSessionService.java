    ...
    private static final String PHOTO_SYSTEM_PROMPT = """
            ...
            2. 약(특히 알약/캡슐)이면:
               ...
               각인(printFront/printBack) 추출 안전 규칙 — 중요:
                 ...
                 * 자신 있는 글자만 보내거나, 자신 없으면 외형(shape/color/line)만으로 검색하세요.

               파라미터 매핑 규칙:
                 * drugShape: 한국어 enum — `원형` / `장방형` / `타원형` / `삼각형` / `사각형` / `마름모형` / `반원형`
               ...
               도구 응답으로 받은 후보(0~10건)에 따라:
                 * 0건 — "외형으로 일치하는 약을 찾지 못했어요. 각인이 잘 보이게 다시 찍어 주실 수 있을까요?"로 안내.
                 * 1건 — **단정 금지**. "**○○로 보입니다**. 약 봉투/처방전과 일치하는지 한 번 확인 부탁드려요." 표현으로 사용자 검증 유도. 약 봉투가 곁에 있으면 비교, 없으면 약사·보호자 확인 권장. 후속 정보(`getDrugInfo`/`checkDur`)는 사용자가 맞다고 확인 후 보강하거나, 응답에 "맞다면 효능은 …" 식으로 조건부로 덧붙이세요.
                 * 2건 이상 — 후보 약명을 짧은 목록으로 제시하고 "각인/뒷면/색이 더 보이게 다시 찍어 주시면 좁힐 수 있어요." follow-up.
