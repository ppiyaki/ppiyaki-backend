...
public class MedicationCompleteDispatcher {
     ...
     *
     * <p><b>전파: REQUIRES_NEW (poison-batch 격리)</b>. Outbox 워커
     * ({@code OutboxRelayWorker.processBatch})의 배치 트랜잭션에서 호출되지만,
     * 합류(REQUIRED)하지 않고 메시지 단위의 새 트랜잭션을 연다. 합류하면 이 메서드의
     * RuntimeException이 참여 트랜잭션을 rollback-only로 마킹해
     * (globalRollbackOnParticipationFailure) 워커의 catch + recordFailure가 무력화되고,
     * processBatch 커밋 시 UnexpectedRollbackException으로 배치 전체가 롤백되어
     * attempts 증가/백오프/데드레터가 전부 동작하지 않기 때문이다. REQUIRES_NEW에서는
     * 실패가 이 내부 트랜잭션만 롤백시키고 예외로 전파되므로, 워커의 외부(배치) 트랜잭션이
     ...
     * (외부 배치 트랜잭션 1개 + 이 REQUIRES_NEW 트랜잭션 1개). 커넥션 풀 사이즈 산정 시 고려할 것.
     *
     * <p><b>부분 커밋 주의</b>: 이 메서드가 성공하면 알림 record는 그 시점에 커밋된다. 이후 워커의
    ...
    private void collectForSlot(
        ...
        for (final CareRelation relation : relations) {
            ...
            // 새로 저장된 record에 대해서만 푸시 명령 생성 (dedup으로 skip된 건은 푸시도 없어 중복 푸시가 최소화된다)
