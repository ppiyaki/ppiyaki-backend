 ...
 *
 * <ul>
 *   <li>(a) FCM 발송이 실패(예외)해도 알림함 record는 저장되고 메시지는 PROCESSED — 푸시 실패가
 *       record 내구성/outbox 상태에 영향을 주지 않는다.</li>
 *   <li>(b) record 저장·outbox 상태 변경은 릴레이 트랜잭션 안에서 일어나고, FCM 호출은 그 트랜잭션
...
class MedicationCompleteOutboxRelayPushBoundaryTest {
    ...
    @Test
    @DisplayName("(b) record 저장은 릴레이 트랜잭션 안, FCM 호출은 커밋 이후 — send 시점에 활성 tx가 없고 별도 커넥션에서 record/PROCESSED가 보인다")
    void pushIsSentAfterCommit_recordIsSavedInsideTransaction() {
        ...
        // relay 트랜잭션이 아직 열려 있다면 record가 보이지 않아야 하고, 커밋됐다면 보여야 한다.
        ...
        // then — send가 실제로 호출됐고,
        verify(pushSender, times(1)).send(anyString(), any(PushPayload.class));
        // 그 시점에 (1) 릴레이 트랜잭션은 이미 끝나 있었으며 (활성 tx 없음)
